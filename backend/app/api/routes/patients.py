...
from app.schemas.care_pathway import CarePathwayClassifyRequest, CarePathwayClassification
from app.schemas.patient import PatientCreate, PatientOut, PatientUpdate
from app.services.care_pathway_classifier import CarePathwayClassifierService
...
@router.post("/me/classify-pathway", response_model=CarePathwayClassification)
async def classify_own_pathway(
    data: CarePathwayClassifyRequest,
    user: User = Depends(get_current_user),
):
    """Suggest a care pathway from the patient's own free-text/voice
    description during onboarding. This never persists anything and never
    diagnoses — it only returns a suggestion the patient must confirm (or
    change) before it is saved via PATCH /patients/me."""
    result = await CarePathwayClassifierService().classify(data.description, data.language)
    return result
