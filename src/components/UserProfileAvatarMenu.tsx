...
export function UserProfileAvatarMenu({
  ...
  role = "Patient",
  facilityOrLocation = "Location not set",
  ...
  // Editable Form State — no patient-specific defaults here (no phone, no
  // pregnancy week, no vitals); every field starts empty/generic and is
  // filled in only from this account's real data once it loads.
  ...
  const fetchPatientProfile = useCallback(async () => {
    try {
      ...
      if (own) {
        ...
        setCarePathway(own.care_pathway || null);
        ...
        // Only a Maternal Care patient's own pregnancy_week is ever shown —
        // never defaulted for any other pathway.
        if (isMaternalPathway(own.care_pathway) && own.pregnancy_week) {
          setPregnancyWeek(own.pregnancy_week.toString());
        }
      ...
      // No mock-profile fallback: if the authenticated patient record could
      // not be loaded, the form simply stays empty/loading rather than
      // silently showing someone else's (Priya's) data.
    } catch (err) {
    ...
    try {
      ...
      if (patientData) {
        await patientsApi.updateMe({
          ...
          pregnancy_week:
            isMaternalPathway(patientData.care_pathway) && pregnancyWeek ? Number(pregnancyWeek) : null,
          ...
          <div className="p-4 bg-slate-50/80 dark:bg-slate-800/50 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px] bg-white dark:bg-slate-800 p-2.5 rounded-xl border border-slate-200/80 dark:border-slate-700/80">
              ...
              <div className="flex items-center gap-1.5 text-slate-700 dark:text-slate-300">
                ...
                <span className="font-medium">Age: {age ? `${age} yrs` : "—"}</span>
              </div>
              <div className="flex items-center gap-1.5 text-slate-700 dark:text-slate-300">
                <HeartPulse className="w-3 h-3 text-rose-600 shrink-0" />
                <span className="font-bold text-rose-700 dark:text-rose-400">
                  BP: {systolicBp && diastolicBp ? `${systolicBp}/${diastolicBp}` : "—"}
                </span>
              </div>
              <div className="flex items-center gap-1.5 text-slate-700 dark:text-slate-300">
                <Activity className="w-3 h-3 text-teal-600 shrink-0" />
                <span className="font-medium">Pulse: {pulse ? `${pulse} bpm` : "—"}</span>
              </div>
            </div>
          </div>
