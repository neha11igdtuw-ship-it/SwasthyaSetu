...
// Neutral placeholder used only until a real screening result exists — must
// not assume any patient is high-risk or maternal by default.
...
export function AppStateProvider({ children }: { children: React.ReactNode }) {
  ...
  // Demo/dev-only seed for the health-worker mock screens (see mockData.ts).
  // Real authenticated patients never read this as a default — the patient
  // screening/symptoms pages only use it when the page is opened directly
  // with no query params at all (no submission just happened), and even
  // then patient/screening/page.tsx gates the pregnancy week on the
  // patient's actual care pathway before ever displaying it.
  ...
  const updatePatientScreening = (
    patientId: string,
    data: {
      riskLevel: "High Risk" | "Watch / Moderate" | "Low Risk";
      bp: string;
      week?: number;
      symptoms: string[];
      carePathway?: string;
    ...
    const match = matchReferralFacility({
      ...
      // Only treat this as a maternal referral if a pregnancy week was
      // actually supplied or the pathway was explicitly maternal — never
      // default every screening submission to Maternal Care.
      carePathway: data.carePathway || (data.week ? "Maternal Care" : "General Primary Care"),
    });
