...
function ScreeningContent() {
  ...
  const queryPathway = searchParams.get("pathway") || "";
  const fromThisCheck = Boolean(queryRisk && queryBp);
  // Only show/consider a pregnancy week when this screening actually belongs
  // to a Maternal Care case — never assumed for any other pathway.
  const isMaternalCase = fromThisCheck
    ? isMaternalPathway(queryPathway)
    : Boolean(queryWeek) || isMaternalPathway(queryPathway);
  ...
  const week = isMaternalCase ? queryWeek || (fromThisCheck ? "" : screeningResult.week) : "";
  ...
  const liveMatch = matchReferralFacility({
    ...
    carePathway: fromThisCheck ? queryPathway || "General Primary Care" : "General Primary Care",
  });
  ...
  const reasons = fromThisCheck
    ? [
        `${bpLine}${thisCheck.highBp ? "" : ` — ${t("bpWithinNormalRange")}`}`,
        ...
        isMaternalCase && thisCheck.flags.bleeding ? t("vaginalBleedingDischarge") : null,
        thisCheck.flags.abdominalPain ? t("severeAbdominalPain") : null,
        isMaternalCase && thisCheck.flags.reducedFetalMovement ? t("reducedFetalMovement") : null,
    ...
    : latestScreening
      ...
      : [bpLine, t("persistentHeadache"), t("swellingFaceHandsFeet")];
