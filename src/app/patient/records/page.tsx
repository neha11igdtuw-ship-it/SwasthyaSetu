      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200/80 dark:border-slate-700 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 block">{t("pregnancyWeek")}</span>
            <span className="text-sm font-extrabold text-slate-900 dark:text-white">
              Week {p.profile.pregnancyWeek}
            </span>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 block">{t("bloodPressureReading")}</span>
            <span className="text-sm font-extrabold text-rose-700">
              {p.profile.vitals.bp}
            </span>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 block">{t("hemoglobin")}</span>
            <span className="text-sm font-extrabold text-rose-700">
              {p.profile.vitals.hemoglobin}
            </span>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 block">{t("callASHA")}</span>
          ...
          {p.timeline.map((vis) => (
            <div key={vis.id} className="relative pl-10 space-y-1">
              ...
              <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800 space-y-2">
                ...
                <div className="flex gap-3 text-[11px] text-slate-500 dark:text-slate-400 font-medium pt-1">
