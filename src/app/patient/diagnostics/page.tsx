...
export default function PatientDiagnosticsPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <PageHeader
        title="Lab Tests & Health Checks"
        subtitle="Track recommended health tests and report availability"
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-600">
              <div className="flex items-center gap-2">
                ...
                <span>Health Center / Lab: <strong>{diag.facilityName}</strong></span>
              </div>
              {diag.scheduledDate && (
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-teal-700 shrink-0" />
                  <span>Scheduled: <strong>{diag.scheduledDate}</strong></span>
                </div>
              )}
            </div>
            ...
            {diag.reportSummary && (
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/60 text-xs space-y-1">
                <span className="font-bold text-slate-800 block">Report Summary:</span>
                <p className="text-slate-600">{diag.reportSummary}</p>
              </div>
            )}
